...
def get_missed_raw_dates(cwd):
    ...
    return [
        ...
        if not Path(f'{cwd}/pipeline/staging/raw_data/year={day.year}/month={day.month:02d}/{day.day:02d}.parquet').exists()
    ]
...
def elt_bigquery_to_raw_data(cwd, missing_dates=None):
    ...
    if not missing_dates:
        print('All raw data were filled. No need to extract data from BigQuery.\n',
    ...
    with duckdb.connect() as duck_conn:
        for day in missing_dates:
            ...
            data = client.query(bigquery_query.format(start_date=start_date, end_date=end_date))
            print("Done.")

            output_dir = Path(f'{cwd}/pipeline/staging/raw_data/year={day.year}/month={day.month:02d}')
...
def main(cwd):
    ...
    processed_dates =  elt_bigquery_to_raw_data(cwd=cwd, missing_dates=get_missed_raw_dates(cwd))
    print("####################################################################")
