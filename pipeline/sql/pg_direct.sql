update pypi.metadata m
set importance_score = p.children_core_counts + 0.3*p.children_non_core_counts
    ,updated_at = now()
from pypi.package_connections p
where m.id = p.id
    and m.importance_score is distinct from
        p.children_core_counts + 0.3*p.children_non_core_counts;


insert into pypi.seo_cache (normalized_name, seo_header, is_active_package)
with source as (
    select
        m.*
    ...
    left join pypi.seo_cache cache using (normalized_name)
    where (cache.normalized_name is null and m.is_active_package)
       or (
            cache.normalized_name is not null
            and (
                m.updated_at > cache.updated_at
                or p.updated_at > cache.updated_at
                or m.is_active_package is distinct from cache.is_active_package
            )
       )
),
prepared as (
...
on conflict (normalized_name) do update set
    seo_header          = excluded.seo_header
    ,is_active_package  = excluded.is_active_package
    ,updated_at         = now()
where pypi.seo_cache.seo_header is distinct from excluded.seo_header
or pypi.seo_cache.is_active_package is distinct from excluded.is_active_package;
